...
public class BLineExample {
    ...
    public Command createSimplePathExample() {
        // 创建起点（位置 + 旋转）
        Path.Waypoint start = new Path.Waypoint(
            new Translation2d(0.5, 0.5),  // 起始位置
            Rotation2d.kZero                // 起始旋转
        );

        // 创建终点
        Path.Waypoint end = new Path.Waypoint(
        ...
        return Commands.sequence(
            Commands.runOnce(() -> drive.setPose(path.getStartPose())),
            bline.buildFollowCommand(path)
        );
    }
    ...
    /**
     * 使用自动联盟翻转
     * BLinePathFollower已配置为自动根据红蓝联盟翻转路径
     */
