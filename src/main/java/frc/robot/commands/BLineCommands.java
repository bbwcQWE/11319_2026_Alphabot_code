 ...
 * BLine路径跟随命令工厂类
 * 
 * 提供便捷的方法来创建和执行BLine路径跟随命令，
 * 支持从JSON文件加载路径和编程方式创建路径。
 */
public final class BLineCommands {
    ...
    public static Command followConstrainedPath(
        ...
        Path.PathConstraints constraints = bline.createPathConstraints()
    ...
    public static Command pathSequence(
        ...
        for (int i = 0; i < paths.length; i++) {
            final int index = i;
            followCommands[i] = Commands.sequence(
    ...
    public static Command continuousPathSequence(
        ...
        for (int i = 0; i < paths.length; i++) {
            final int index = i;
            followCommands[i] = Commands.deadline(
                bline.buildFollowCommandWithPoseReset(paths[index]),
     ...
     * @return 设置模块方向的命令
     */
    public static Command preorientModules(BLinePathFollower bline, Drive drive, String pathFilename) {
